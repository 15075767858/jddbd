...
function updateNameAndUse(option, callback) {
    var goodsName, goodsUse, paimaiId;
    goodsUse = option.goodsUse;
    goodsName = option.goodsName;
    goodsName = goodsName.replace(/\?/gm, "");
    paimaiId = option.paimaiId;
    //console.log(paimaiId, goodsName)

    pool.query("update " + tableName + " set goodsUse =\"" + goodsUse + "\" , goodsName=\"" + goodsName + "\"  where paimaiId=? ", [paimaiId],
...
function getNullGoods(callback) {
    pool.query("select paimaiId from " + tableName + " where goodsName is null or goodsUse is null  limit 0,300",
        function (err, results, fields) {
            callback(err, results, fields);
        }
    );
...
function getNullCount(callback) {
    pool.query("select count(paimaiId) from " + tableName + " where goodsName is null",
        function (err, results, fields) {
            callback(err, results, fields);
        }
    );
...
exports.getNullCount=getNullCount;